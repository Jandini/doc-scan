...
while true; do
    ...
    else temp=/tmp/doc_scan/$(date +%Y%m%d_%H%M%S); fi

    batch=$(date +%Y%m%d_%H%M%S)
    mkdir -p $temp
    scanimage --format tiff --batch-count 2 --batch=$temp/$(date +%Y%m%d_%H%M%S)_%04d.tif --resolution $RESOLUTION --mode $MODE --source "ADF Duplex"
